        ...
        ((croak (msg &rest args)
         ...
         (read-symbol ()
           (let ((sym (read-symbol-chunk)))
             (handler-case
                 (list :num (parse-number:parse-number sym))
               (error ()
                 (list :sym sym)))))
         ...
         (read-comma ()
           ...
           (cond
             ((char= (peek) #\@)
              (next)
              (list :splice (read-token)))
             (t
              (list :unquote (read-token)))))
         ...
         (read-token ()
           ...
           (let ((ch (peek)))
             (cond
               ...
               ((char= ch #\") (list :str (read-string)))
               ((char= ch #\() (list* :list (read-list #\))))
               ((char= ch #\[) (list* :list (read-list #\])))
               ((char= ch #\') (read-quote))
               ((char= ch #\`) (read-qq))
               ((char= ch #\,) (read-comma))
           ...
           (loop for ch = (peek) with ret = '()
              do (cond
                   ...
                   ((char= ch *token-stop*)
                    (return (nreverse ret)))
                   (t
                    (push (list :cat (read-text-chunk)) ret))))))
